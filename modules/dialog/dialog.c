...
static int dialog_subscribe(struct dialog *dialog)
{
	...
	err = sipevent_subscribe(&dialog->sub, uag_sipevent_sock(),
				 c_uri, NULL, aor, "dialog", NULL,
				 account_subint(ua_account(dialog->ua)),
				 ua_cuser(dialog->ua), routev,
				 routev[0] ? 1 : 0, auth_handler,
				 ua_account(dialog->ua),
...
static int dialog_subscribe_all(struct ua *ua)
{
	...
	int err = 0;
	/* We will subscribe to all contacts*/
